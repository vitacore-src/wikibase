...
Действие оператора аналогично действию оператора «IS» (п. 2.4.4) с той разницей, что «ExactIS» возвращает положительное значение только в том случае, если атрибут-ссылка указывает именно на ту сущность, которая задана во втором операнде. Т.е. ссылка на сущность-наследника игнорируется.
...
Диалог содержит список атрибутов (см. п.3 Руководства), в котором видны только атрибуты, являющиеся коллекциями. Из списка необходимо выбрать атрибут, соответствующий child-сущности (значок ![97](/uploads/000003/97.png "97") , см. раздел 1.1 Руководства).
...
2.7	Команда «Упорядочить параметры»
Если в выражении используется большое количество пользовательских параметров (см. п. 2.3.4) существует возможность настраивать порядок их ввода для удобства пользователя – например, их можно сгруппировать по типам. Для этого предназначена кнопка   управляющей панели генератора. Эта кнопка доступна, только если в редактируемом выражении присутствуют пользовательские параметры.
...
2.8	Окно редактирования XML
Переход в режим создания условий с помощью языка XML включается кнопкой   на управляющей панели формы. При этом область формы ниже управляющей панели занимает поле редактирования текста, в котором пользователь может самостоятельно набрать XML-код (см. Рис. 2.25).
Этот же режим может быть использован для просмотра XML-кода условия, созданного с помощью графического конструктора.
Созданный в этом окне код может быть сохранен с помощью кнопки   в виде файла XML. Содержимое ранее сохраненного файла может быть загружено в окно редактирования с помощью кнопки  .	 
Рис. 2.25 Форма редактора XML-кода

Переключение в режим отображения XML-кода невозможно, если выражение содержит ошибки, т.е. некорректные элементы. Приложение при этом выдает сообщение об ошибке. Возможность перехода в XML-редактор появится, если временно отключить режим проверки корректности выражения кнопкой   управляющей панели генератора выражений. При этом некорректные элементы выражения не будут включены в XML-код.
...
3.	Форма списка атрибутов сущности
...
Рис. 3.1. Форма отображения списка
 атрибутов сущности

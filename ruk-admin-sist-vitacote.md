<!-- TITLE: АИС "ОМС Финанс" с правами Администратор-->
...
[video](https://youtu.be/Q2FtFArSv9A){.youtube}
...
[video](https://youtu.be/IJTIxTFvTmg){.youtube}
...
[video](https://youtu.be/jFnHiKiO4e4){.youtube}
...
|(Список ролей)|	Список всех доступных ролей системы с возможностью отметки. Список ролей, предусмотренных для работы с системой ОМС-Финанс, описан в [ п. 1.5.1](#a35) настоящего руководства.|
|Медицинское учреждение\*|	Поле выбора МО из справочника ([см. п. 2.1](#a1)) для пользователя медицинской организации.| 
|Страховая организация\*|	Поле выбора СМО из справочника ([см. п. 2.3](#a11)) для пользователя страховой медицинской организации.|
|Филиал ТФОМС\*|	Поле выбора филиала ТФОМС из справочника ([см. п. 2.2](#a10)) для пользователя территориального фонда (или его филиала).|
...
Для организации корректного доступа к данным МО пользователей ТФОМС также должны быть сделаны настройки соответствия филиалов фонда и курируемых им МО ([см. п. 2.2](#a10)).
...
Здесь в первом поле формы отображаются данные информационного сообщения, к которому прикрепляется документ, второе поле позволяет выбрать документ из списка нормативных документов, загруженных в систему (ведение списка нормативных документов описано в [п. 10.3](#a34)).
Если в списке отсутствует требуемый документ, для его добавления необязательно переходить в папку нормативной документации, можно добавить его тут же, в открывшемся списке (кнопка добавления документа доступна в контекстном меню табличной формы, далее все действия соответствуют описанным в [ п. 10.3](#a34)). Добавленный таким образом документ будет затем виден в папке «Нормативная документация» панели структуры приложения.
...
Форма редактирования в целом аналогична форме редактирования данных информационного сообщения, описанного в [п. 10.2](#a36) После заполнения и сохранения данных формы документ с описанием и вложением станет доступен в списке нормативной документации.
...
|Тип|Поле «Тип» содержит следующие варианты выбора: «Ошибка» или «Предупреждение».Подробное описание	Поле для ввода описания сути проверки. Следует иметь в виду, что данный текст будет включаться в текст сообщения об ошибке, которое увидит пользователь (см. описание пользовательской валидации в [п. 6.2.2](#a30)).|
...
![68](/uploads/002/68.png "68")
 ...
 Вкладка «Фильтр» содержит панель редактора выражений, с помощью которого можно задать условие отбора данных базовой сущности для формирования данной таблицы. Каждая строка полученной выборки будет представлена строкой в результирующей таблице, если не задано условие группировки ([см.п. 10.5.4](#a38)). 
 
 ### Настройка сортировки данных
 
 Настройка порядка сортировки данных в результирующей таблице производится на вкладке «Сортировка» формы настройки таблицы экспорта (см. Рис.).

![69](/uploads/002/69.png "69")
 
 На данной вкладке отображается список атрибутов, используемых для сортировки данных в выборке данных для экспорта, и кнопки для управления этим списком. Кнопки управления списком аналогичны описанным в [п. 10.5.1](#a39) Настройка колонок таблицы. При добавлении атрибута в список открывается форма выбора атрибута, показанная на Рис. Выбор атрибута производится с помощью редактора выражений. Порядок следования атрибутов в списке определяет последовательность сортировки значений данных атрибутов в выборке.

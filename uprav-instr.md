...
Только для отчетов MS Word, MS Excel и Html форматов. Гиперссылки используются для реализации идеи "активных" отчетов, суть которой в сохранении связи отчета с приложением, после его генерации. Связь реализуется стандартными гиперссылками: при активации пользователем гиперссылки в документе, в клиентском приложении открывается форма объекта, на который гиперссылка ссылается. В url гиперссылки используется кастомный protocol prefix (стандартные, например, http, tcp).
```
url="customprefix:\\Report 9ca76f86-8967-4b72-9c71-78ee61bfd49c"
```

Прикладной protocol prefix настраивается на исполняемый файл клиентского приложения в реестре Windows. Для разных приложений используются разные значения protocol prefix. Изменения в реестр вносятся либо вручную, либо с помощью инсталлятора клиенского приложения, как описано в разделе "Применение в кастомных проектах".

```
Windows Registry Editor Version 5.00

[HKEY_CLASSES_ROOT\customprefix]

@="Custom protocol"

"Url protocol"=""

[HKEY_CLASSES_ROOT\customprefix\shell]

[HKEY_CLASSES_ROOT\customprefix\shell\open]

[HKEY_CLASSES_ROOT\customprefix\shell\open\command]

@="\"D:\\Program Files\\EsterDev\\AST Client\\AST.UserArm.exe\" \"%1\""
```

При активации гиперссылки операционная система стартует указанное в реестре приложение с командной строкой заданой в гиперссылке. В Url гиперссылки за префиксом следует наименование сущности и через пробел значение первичного ключа, по этим данным в клиенском приложении открывается форма указанного бизнес-объекта.

```
url="customprefix:\\Report 9ca76f86-8967-4b72-9c71-78ee61bfd49c"
```

### Строковое представление в шаблоне

```
%%hlink(путь_к_отображаемому_атрибуту, путь_к_объекту,
путь_к_атрибуту_всплывающая_подсказка)%%
...
Строковое представление лексемы содержит от одного до трех параметров.

1 параметр – обязательный, задает лексему, значение которой показывается в теле документа. Именно это значение в документе помечается как гиперссылка. 

2 параметр – не обязательный, задает путь к объекту, форма которого открывается в приложении при активации гиперссылки. Если параметр не задан, то ссылочный объект определяется либо по 1-ому параметру (если он лексема Path с количеством элементов Step более одного), либо используется текущий объект { . }. 

3 параметр – не обязательный, задает лексему, значение которой используется в качестве всплывающей подсказки (screentip) на гиперссылке. 

### Применение в кастомных проектах

**Для подключения гиперссылок к кастомному проекту необходимо:**

Задать значение HyperlinkPrefix в секции GEst.Report конфигурационных файлов клиентского и серверного приложений 

Включить код проверки командной строки в начало процедуры старта клиентского приложения. Поместить его внутрь блока try…catch. 
...
6.	
    {
7.	
            if (GEst.Client.PrefixProtocolHandler.CheckAlreadyStarted(args))
8.	
            {
9.	
                    return;
...
14.	
    {
15.	
            GEst.Client.ExceptionHandler.Catch(ex);
16.	
    }
17.	
}           
...
Внести изменения в ClientInstaller. В раздел Custom Actions, в секции Install и Uninstall включить "Primary output from GEst.Client". Регистрация прикладного protocol prefix в реестре произойдет только после инсталляции клиентского приложения. 

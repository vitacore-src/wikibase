...
![81](/uploads/000001/81.png "81")
 ...
 Следующим шагом необходимо нажать кнопку ,  ![82](/uploads/000001/82.png "82") после чего на экране появится окно, в котором следует выбрать нужные значения и нажать <ОК>,   как это показано на рисунке ниже.
...
Во вкладке <Исключающие правила> можно настроить правила для протокола. Они применяются только при использовании кнопки <Закрепить значения> и позволяют исключить те или иные позиции заявок для того, чтобы после формирования протокола их значения не изменились, а сохранились прежние объемы. 
...
После чего данное правило появится в списке предыдущего окна (Рис.4), где следует нажать  кнопку <Ок>, после этого правило будет отображаться в табличной форме основного окна вкладки  <Исключающие правила>, а затем нужно нажать на кнопку  ![5 4](/uploads/00003/5-4.jpg "5 4") , чтобы сохранить созданное правило.

Для просмотра уже созданного правила необходимо перейти во вкладку <Исключающие правила>, нажать кнопку ![5 5](/uploads/00003/5-5.jpg "5 5") , в открывшемся окне выбрать нужное правило, нажать правую кнопку мыши, в появившемся контекстном  меню выбрать пункт <Свойства>. Таким образом появится окно, в котором отображается условие, применяемое к данному правилу, как показано на рисунке ниже. 
...
Программой предусмотрена возможность редактирования значений объемов с помощью процентов по разделам планирования в протоколе. Применение данной функции позволяет увеличивать или уменьшать значения объемов определенным медицинским организациям, по выборочным условиям оказания или профилям с помощью процентов. 

Для того чтобы редактировать значения в процентах пользователю необходимо в основном окне протокола перейти во вкладку <Рассмотрение>, где следует отметить "флажком" разделы планирования, по  которым нужно изменить объемы, а также в табличной форме настроить требуемые фильтры и нажать кнопку ![80](/uploads/000001/80.png "80") , как это показано на рисунке 7.3.1. 

 Следующим шагом необходимо нажать кнопку , ![82](/uploads/000001/82.png "82")
после чего на экране появится окно, в котором следует выбрать нужные значения и нажать <ОК>, как это показано на рисунке 7.3.2
...
Функция изменения значений будет применена к тем позициям, которые будут отображены в окне программы на момент ее настройки.  В табличной форме окна измененные значения будут выделяться - эти ячейки окрашены в голубой цвет, а сами значения синего цвета, как показано на рисунке 7.3.3
...
Далее в открывшемся окне нужно нажать на «Загрузить». После завершения загрузки данных, следует выбрать «Обновить список».
...
Ниже располагаются поля фильтра, позволяющие отобразить в списке только позиции, удовлетворяющие заданному условию. Правила пользования фильтрами и способы управления данными с помощью табличной формы (фильтрация, сортировка, группировка) аналогичны описанным для формы утверждения заявки МО (см. п. 6.4, 6.5).

Основную часть формы занимает табличная форма для просмотра и редактирования значений нормативов. По умолчанию список нормативов отсортирован в порядке следования колонок таблицы – по МО и далее в порядке следования разделов планирования.
...
Для упрощения ввода можно воспользоваться фильтрами в верхней части формы (см. применение фильтров в п. 6.4). В режиме наложенного фильтра значения фильтра используются в качестве значений по умолчанию для вновь добавляемых строк плана. На рисунке ниже это схематично показано.
...
Попытка удаления каких-либо данных вызывает появление диалогового окна с запросом подтверждения удаления (см. Рис. 11.2.1). Это делается для снижения вероятности случайного удаления данных. Появление запроса позволяет проверить уверенность пользователя в выбранном действии.
 
 ![8](/uploads/000001/8.png "8")
 
Если в системе существуют данные, связанные с удаляемым объектом, удаление невозможно, и в таком случае будет выведено сообщение об ошибке (см. п. 11.3), описание ошибки содержит текст «Есть связанные объекты…».
...
В ряде случаев система запрашивает у пользователя значения параметров, необходимых для выполнения какой-либо процедуры, например, при наложении фильтра на данные табличной формы или формировании отчета. Диалог запроса параметров выглядит так, как это показано на Рис. 10.4.
...
Диалоговое окно настройки внешнего вида списка показано на рис. 12.3.  В верхней части окна слева отображается наименование сущности, для которой настраивается форма.
...
При работе с большими объемами данных возникает необходимость скрыть ненужную в данный момент информацию. Для этого используются фильтры. Фильтр содержит набор условий, которые накладываются на значения атрибутов сущности, представленной в табличной форме. Панель фильтра всегда присутствует на универсальной табличной форме и занимает верхнюю часть области данных открытой вкладки (см. Рис. 12.1 Универсальная табличная форма).
...
После нажатия кнопки «Сброс» условия фильтра удаляются и, если определенные условия фильтра используются неоднократно, их можно сохранить для последующего применения. Функции, связанные с сохранением фильтров, описаны в п.11.3.3 настоящего руководства.
В условиях фильтра может быть применен параметр, значение которого требуется указать в момент наложения фильтра (см. п.11.3.2). В таком случае при применении фильтра кнопкой <Найти> на экране появится диалоговое окно запроса значения параметра.
...
Опция «Настройка» открывает диалог управления списком сохраненных фильтров (см. Рис. 12.). С помощью данного диалога можно удалить ненужные фильтры.

## 	Функции работы с данными

Универсальная табличная форма включает функции, связанные с обработкой данных списка. Их перечень представлен в Табл. 12.1.
...
|Экспортировать объекты списка| ![4445](/uploads/000001/4445.jpg "4445") |Функция позволяет выгрузить данные списка в документ, сохраняемый на компьютере пользователя. Описание функции экспорта приведено в п. 12.5.|
...
Формы вложенных списков используются для отображения различных списков, встроенных в формы редактирования, например, список полисов пациента, спецификация накладной и т.п. Такой список обладает ограниченной функциональностью по сравнению с универсальной табличной формой, описанной в п. 11. Вложенные списки имеют несколько различных реализаций. В данном разделе описаны общие принципы их функционирования. 
...
|Удалить|[Delete]|![26 3](/uploads/000001/26-3.jpg "26 3")|Выделенный в момент нажатия кнопки элемент списка удаляется. При этом выводится стандартный запрос подтверждения удаления (п. 10.2). Если существуют связанные с выбранным элементом данные, удаление невозможно и будет выведено соответствующее сообщение.|
...
Финансовый план- ввод сводных плановых средств по территориальной программе обязательного медицинского страхования с разбивкой на год по месяцам. 
...
Общий внешний вид редактирования финансового плана представлен на рисунках ниже.

![29](/uploads/000001/29.png "29")

Пользователю программы ОМС «Финанс» необходимо выбрать интересующий его отчетный период.  Для отображения списка требуется выбрать нужное значение в поле <Отчетный период>.  Также можно сразу задать нужное условие отбора данных с помощью полей фильтра. 

![30](/uploads/000001/30.png "30")

После нажатия кнопки <Показать> отображается полный перечень созданных случаев/обращений, действующий в указанном периоде времени.

![31](/uploads/000001/31.png "31") ![32](/uploads/000001/32.png "32")

Ниже располагаются поля фильтра, позволяющие отобразить в списке только позиции, удовлетворяющие заданному условию. Правила пользования фильтрами и способы управления данными с помощью табличной формы (фильтрация, сортировка, группировка) аналогичны описанным для формы утверждения заявки МО (см. п. 6.4, 6.5).

Основную часть формы занимает табличная форма для просмотра и редактирования значений финансового плана. По умолчанию список отсортирован в порядке следования колонок таблицы – по МО и далее в порядке следования разделов планирования.

Управление списком производится с помощью кнопок «Добавить» и «Удалить», расположенных справа над табличной формой. При нажатии кнопки добавления создается пустая строка, в которой требуется ввести нужные значения. На рисунке ниже это представлено.

![33](/uploads/000001/33.png "33")

Следующий этап- это выбор необходимой медицинской организации.

![34](/uploads/000001/34.png "34")

Далее необходимо выбрать <Тип финансирования>. 

![35](/uploads/000001/35.png "35")

Следующим этапом заполнения полей фильтра является <Выбор медицинской помощи>.

![36](/uploads/000001/36.png "36")

Так же следует выбрать необходимый <Тип профиля> и соответствующий <Профиль>.

Пример представлен на рисунках ниже.

![37](/uploads/000001/37.png "37")

![38](/uploads/000001/38.png "38")

![40](/uploads/000001/40.png "40")

После заполнения полей фильтров (описание выше) необходимо добавить пустую строку для внесения денежной суммы, осуществляется это с помощью кнопок «Добавить» и «Удалить», расположенных справа над табличной формой. Пользователю необходимо внести денежную сумма в эти строки в помесячной разбивке (как показано на картинке выше).

 ![5 4](/uploads/00003/5-4.jpg "5 4") с помощью данной кнопки все введенные значения сохраняются в БД, рекомендуется периодически сохранять данные.
#  Расчет стоимости территориальной программы на регионы.
...
Перед тем как перейти к созданию нового расчета ТП пользователю следует создать записи по видам МП. Для этого заходим в пункт меню <Сервис> - <Просмотр произвольного списка> и находим сущность <TP_TYPE> как это показано на рисунке ниже:  

![41](/uploads/000001/41.png "41")

![42](/uploads/000001/42.png "42")


Рис.16.1.1.  Поиск сущности для создания видов МП.
Открытие формы производится кнопкой <Ок> и в открывшемся списке следует создать новые записи через панель действий, либо нажатием правой кнопкой мыши как показано на рисунке:

![43](/uploads/001/43.png "43")

![44](/uploads/000001/44.png "44")

Поля обязательные для заполнения:
* 	**Наименование** – указывается наименование МП.
* 	**Приоритет** – задается приоритет в общем списке (начинается с 0). Для подчиненных видов МП приоритет следует задавать после основного вида МП.
* 	**Активный** – выбор статуса записи.

Оставшиеся поля:
* 	**Основной вид МП** – выбор из созданного списка вида МП. Это поле заполняется тогда, когда требуется отразить в какой вид МП входит создаваемая запись. 
* 	**Программа ОМС** – выбор программы ОМС.
* 	**Условия оказания МП** – выбор условий оказания МП.
* 	**Вид МП** – выбор вида МП.
* 	**Профиль** – выбор профиля.
* 	**КСГ (Группы ВМП)** – выбор КСГ.

После добавления всех необходимых видов МП пользователю можно приступить к работе по созданию расчета стоимости ТП.

## 	Работа со списком расчетов стоимости ТП.

На панели структуры приложения пользователю необходимо выбрать папку **<Экономика>**. 
Двойной клик мышью на заголовке раздела, открывает папки, как показано на рисунке 4.
Для работы следует выбрать папку **<Расчет стоимости ТП>**.

Норматив финансового обеспечения ТП ОМС может превышать установленный БП ОМС норматив финансового обеспечения БП ОМС в случае:
* 	установления органом государственной власти субъекта РФ дополнительного объема страхового обеспечения по страховым случаям, установленным БП ОМС, 
* 	установления перечня страховых случаев, видов и условий оказания МП в дополнение к установленным БП ОМС. 

![45](/uploads/000001/45.png "45")

Стоимость утвержденной ТП ОМС не может превышать размер бюджетных ассигнований на реализацию ТП ОМС, установленный законом субъекта РФ о бюджете ТФОМС.

Двойной клик по данной папке разворачивает окно, которое показано на рисунке ниже. Добавление нового элемента списка производится нажатием правой кнопки мыши.

![46](/uploads/000001/46.png "46")
...
После создания нового элемента списка, пользователю открывается окно, поля которого необходимо заполнить.  На рисунке ниже это представлено. Если расчет на требуемый период уже существует, виден в списке заявок и доступен для внесения изменений, его можно открыть для редактирования двойным щелчком на нужной записи списка.

![47](/uploads/000001/47.png "47")

## 	Поля обязательные для заполнения:  

* 	**Отчетный период**– указывается период отчета.
* 	**Источник финансирования**– выбор источника получения финансовых ресурсов.
* 	**Дата начала действия**– выбор даты.
* 	**Население** – численность постоянного населения субъекта.
* 	**Субвенции на 1 з.л (руб)** – денежные средства, выделяемые на определенный срок из бюджета вышестоящего уровня для определенных целей на 1 застрахованное лицо в год.
* 	**Содержание ТФОМС (руб)** – средства Территориального Фонда ОМС.
* 	**Содержание СМО (руб)**  -  средства страховой медицинской организации.

![48](/uploads/000001/48.png "48")

Вкладка «Нормативы» описывает вид медицинской помощи, объемы федерального и регионального нормативов, также финансовые затраты федерального и регионального бюджетов, 
и объемы межтерриториальных расчетов. 


![49](/uploads/001/49.png "49")

![50](/uploads/001/50.png "50")

## 16.4	Форма редактирования расчета ТП.

После заполнения обязательных строк, пользователю необходимо нажать кнопку «Продолжить», далее открывается окно.

![51](/uploads/000001/51.png "51")

Верхняя часть формы содержит: 

|  Реквизит формы |Описание |
|:------------- |:---------------|
|Отчетный период|Поле для указания периода, на который формируется расчет ТП.|
|Источник финансирования|Поле для указания источника получения финансовых ресурсов (по умолчанию следует выбрать ОМС, при необходимости выбрать другой источник финансирования).|
|Дата начала действия |Поле для указания даты начала действия ТП.|
|Описание |Поле для краткой характеристики.|
|Население 	|Поле для ввода численности постоянного населения.|
|Субвенции  1 з.л.	|Поле для ввода денежных средств, выделяемых на определенный срок из бюджета вышестоящего уровня для определенных целей на 1 застрахованное лицо в год.|
|Содержание ТФОМС	|Поле для внесения средств Территориального Фонда ОМС.|
|Содержание СМО	|Поле для внесения средств страховой медицинской организации.|
|Показать предыдущий год	|Показывает данные разницы между текущим и предыдущим годами.|
|Сумма субвенций	|Общая сумма субвенций. Рассчитывается путем умножения субвенций на 1 з.л. и численности населения.|
|Нераспределенный остаток 	|Неиспользованные денежные средства, переходящие на следующий год.|


Основную часть формы занимает табличная форма, колонки которой содержат следующие значения – вид медицинской помощи, население, нормативные объемы ( федеральные и региональные), объемы, нормативы финансовых затрат ( федеральные и региональные) , объемы финансирования в состав ,которых входят нормативы объемов федеральные и региональные, прочие поступления, межбюджетные трансферы, подушевой норматив финансирования территориальной программы; нормированный страховой запас (объемы МТР, прочие расходы, расчет НСЗ), финансирование. 

![52](/uploads/000001/52.png "52")

![53](/uploads/000001/53.png "53")

![5 4](/uploads/00003/5-4.jpg "5 4") с помощью данной кнопки все введенные значения сохраняются в БД, рекомендуется периодически сохранять данные.

# 	Коэффициенты.

Объем финансового обеспечения медицинской организации, оказывающей медицинскую помощь, осуществляется по системе КСГ (КПГ) заболеваний на основе следующих экономических параметров:
...
**Коэффициент относительной затратоемкости** – устанавливаемый на федеральном уровне коэффициент затратоемкости клинико-статистической группы заболеваний, отражающий отношение ее затратоемкости к базовой ставке.
...
## 	Работа со списком коэффициентов. 

На панели структуры приложения пользователю необходимо выбрать папку <Экономика>. 
 Двойной клик мышью на заголовке раздела, открывает папки, как показано на рисунке.
 Для работы следует выбрать папку < Коэффициенты>.
 
 ![54](/uploads/001/54.png "54")
 
## Добавление нового списка.

Двойной клик по данной папке разворачивает окно, которое показано на рисунке ниже. Добавление нового элемента списка производится нажатием правой кнопки мыши. 

 ![55](/uploads/001/55.png "55")
...
После создания нового элемента списка, пользователю открывается окно, поля которого необходимо заполнить.  На рисунке ниже это представлено.

![56](/uploads/001/56.png "56")

## 	Поля для заполнения: 

**Наименование** - поле для указания названия коэффициента.
**Медицинская организация** - поле для выбора медицинской организации.
**Тип коэффициента** - поле для выбора коэффициента (простой, процентный). 
     Простой коэффициент-значение коэффициента для всех случаев. 
     Процентный коэффициент - применим только для определенного процента от общего числа                 случаев. 
**Значение коэффициента** - поле для указания числового показателя коэффициента.
**Вычисляемый коэффициент** - поле для указания группирующего условия.
**Дата начала действия** - поле для указания с какого периода действует данный коэффициент.
**Дата окончания действия** - поле для указания срока действия данного коэффициента
**Приоритет** – поле для указания преобладающего коэффициента (в зависимости от данного поля будет применяться тот или иной коэффициент).
**Процент** - поле для указания процента коэффициента.
**Количество знаков при округлении** - данное заполняется автоматически.
**Вычислимый** - при нажатии данной кнопки поле <Вычисляемый коэффициент> становится доступным для выбора условия.
**Срезы статистики** - поле для выбора до какого уровня будет отображена статистика.
**Управление статистикой** - данный раздел позволяет отслеживать статистику за определенный срок. 

## 	Работа с вычисляемым коэффициентом.

Для того, чтобы поле <Вычисляемый коэффициент>  стало доступным для выбора группирующего условия пользователю необходимо нажать кнопку <Вычислимый>.  Как показано на рисунке ниже.

![57](/uploads/001/57.png "57")

После нажатия кнопки <Установить> открывается окно с панелью фильтра. Использование фильтров подробно описывает Руководство пользователя - Редактор выражений.  На рисунке ниже это представлено.

![58](/uploads/001/58.png "58")

![59](/uploads/001/59.png "59")

## 	Управление статистикой.

**Управление статистикой** - данный раздел позволяет отслеживать статистику за определенный срок, возможны два способа отображения статистики:
 1) интеграция с программным продуктом АИС МЕДЭКСПЕРТ
2) ручной ввод- добавление нового элемента производится нажатием правой кнопки мыши. 

![60](/uploads/001/60.png "60")

Чтобы увидеть статистику по  выбранному коэффициенту пользователю следует нажать кнопку <Управление статистикой>, открывается окно, как показано на рисунке ниже. 

1) интеграция с программным продуктом АИС МЕДЭКСПЕРТ.

![61](/uploads/001/61.png "61")


2) ручной ввод- добавление нового элемента производится нажатием правой кнопки мыши. 

![62](/uploads/001/62.png "62")

После внесения всех необходимых данных, пользователю следует сохранить все значения с помощью данной кнопки ![M 6](/uploads/007/m-6.jpg "M 6") .

# 	Расчет за единицу объема.

На панели структуры приложения пользователю необходимо выбрать папку <Экономика>. 
 Двойной клик мышью на заголовке раздела, открывает папки, как показано на рисунке.
 Для того, чтобы начать работу с разделом <Расчет финансового плана> пользователю следует настроить <Разделы финансового плана и Конфигурацию финансового плана>.


![63](/uploads/001/63.png "63")

## 	Работа с настройками разделов финансового плана.

Пользователю на панели структуры необходимо развернуть двойным нажатием мыши папку **<Настройки>**. Далее следует выбрать папку** <Разделы финансового плана>**, открывается окно, как показано на рисунке ниже.  Раздел - это группа объектов объединенных определенным условием,   для которых будет формироваться расчетная стоимость.  

![64](/uploads/001/64.png "64")

Создание нового раздела финансового плана производится с помощью правой кнопки мыши. На рисунке ниже представлен пример создания нового раздела.

![65](/uploads/001/65.png "65")

При создании нового раздела пользователю открывается окно, в котором необходимо заполнить поля – Наименование, уровень детализации, а также следует указать условие (фильтрами тип профиля). На рисунке ниже представлен пример с заполненными полями.

![66](/uploads/001/66.png "66")

Во вкладке **«Группы»** необходимо указать код уровня детализации раздела планирования для их отображения в колонке “Тарифы” табличной формы финансового плана (как показано на рис.ниже).

![67](/uploads/001/67.png "67")

Во вкладке **<Связанные записи из расчета ТП>** необходимо указать связанный вид МП для расчета ТП.  То есть за счет, каких выделенных средств, будет производиться расчет. 
Добавление новой записи производится с помощью кнопки ![67 1](/uploads/001/67-1.png "67 1") , удаление - ![67 2](/uploads/001/67-2.png "67 2") .

![68](/uploads/001/68.png "68")

Так же пользователю следует во вкладке **<Коэффициенты>** добавить  интересующий его коэффициент. Добавление производится из уже существующего справочника коэффициентов, с помощью  ![68 1](/uploads/001/68-1.png "68 1")
![67 1](/uploads/001/67-1.png "67 1")- Добавление нового коэффициента.
![67 2](/uploads/001/67-2.png "67 2") - Удаление коэффициента.
![68 2](/uploads/001/68-2.png "68 2")- Изменение положения коэффициента в списке ( вверх).
![68 3](/uploads/001/68-3.png "68 3")    - Изменение положения в списке (вниз).
	
![69](/uploads/001/69.png "69")
	
![70](/uploads/001/70.png "70")
	
Для отображения плановых показателей по возрастным группа Взрослые и Дети в табличной форме финансового плана в колонках Расчет Финансирования необходимо произвести следующие настройки. На панели структуры приложения пользователю необходимо выбрать папку **<Настройки планирования>**, далее **<Параметры планирования>**.  
		
![71](/uploads/001/71.png "71")
		
В созданных параметрах планирования необходимо отобразить возрастную группу, как показано на рисунке ниже.
		
![72](/uploads/001/72.png "72")
		
После чего необходимо нажать на ![5 4](/uploads/00003/5-4.jpg "5 4")  , чтобы сохранить данные.

Далее на форме создания «Раздела финансового плана» пользователю необходимо поставить галочку **<Разбивать по возрастным группам>**, чтобы появилась вкладка **<Настройки возрастных групп>**. 

![73](/uploads/001/73.png "73")

Данная вкладка содержит условия для передачи значений тарифов оказываемых услуг по возрастным группам (например, «Взрослые», «Дети», «Все») при расчете финансового плана.
Чтобы добавить условие пользователю во вкладке **<Настройка возрастных групп>** необходимо нажать ![67 1](/uploads/001/67-1.png "67 1") на панели инструментов, затем выбрать соответствующую возрастную группу, как на Рис. 18.1.8.

![74](/uploads/001/74.png "74")

Далее, необходимо дважды кликнуть левой кнопкой мыши по выбранной возрастной группе, как на Рис. 18.1.9.

![75](/uploads/001/75.png "75")

Далее, в появившемся окне пользователю в поле <Применить для> необходимо выбрать «Тариф», а после с помощью фильтров нужно сформировать условия для каждой возрастной группы, пример заполнения показан на Рис. 18.1.10. 

![76](/uploads/001/76.png "76")

Далее нажать <Ok> и сохранить данный раздел планирования.
Значения тарифов по возрастным группам будут переданы из справочника <Тарифы\Базовые ставки> и отображены в табличной форме финансового плана, колонки которого содержат значения «Тарифы», как показано на Рис. 18.1.11.

![77](/uploads/001/77.png "77")

## 	Работа с конфигурацией финансового плана.
Следующим этапом настройки **<Расчета финансового плана>** является настройка **<Конфигурации финансового плана>**.  Данный раздел находится в папке **<Настройки>**.

![78](/uploads/001/78.png "78")

Двойной клик по данной папке разворачивает окно, которое показано на рисунке ниже. Добавление нового элемента списка производится нажатием правой кнопки мыши. 

![79](/uploads/001/79.png "79")

После создания нового элемента списка, пользователю открывается окно, поля которого необходимо заполнить.  На рисунке ниже это представлено.

![80](/uploads/001/80.png "80")


Пользователю системы следуют указать наименование, так же необходимо добавить Раздел финансового плана. 
**!!! Важно.*** Для расчета одного финансового года, необходимо все разделы добавлять именно в данную конфигурацию. 
 Добавление нового раздела финансового плана производится с помощью  ![80 1](/uploads/001/80-1.png "80 1")
 ![67 1](/uploads/001/67-1.png "67 1") - Добавление нового коэффициента.
 ![67 2](/uploads/001/67-2.png "67 2")- Удаление коэффициента.
 ![68 2](/uploads/001/68-2.png "68 2")- Изменение положения коэффициента в списке ( вверх).
 ![68 3](/uploads/001/68-3.png "68 3")-Изменение положения в списке (вниз).
 
 ![81](/uploads/001/81.png "81")

![82](/uploads/001/82.png "82")
...
Финансовый план - ввод сводных плановых средств по территориальной программе обязательного медицинского страхования с разбивкой на год по месяцам.
 ...
 На панели структуры приложения пользователю необходимо выбрать папку **<Экономика>**. 
 Двойной клик мышью на заголовке раздела, открывает папки, как показано на рисунке.
 Для работы следует выбрать папку **< Расчет финансового плана>**.
 
 ![83](/uploads/001/83.png "83")
 
 Двойной клик по данной папке разворачивает окно, которое показано на рисунке ниже. 
 
 ![84](/uploads/001/84.jpg "84")
 
 Следующим шагом необходимо нажать кнопку ![84 1](/uploads/001/84-1.png "84 1") , в результате чего появится форма с полями для выбора параметров, которые необходимо заполнить, как показано на рисунке ниже. 

![85](/uploads/001/85.png "85")

При создании нового финансового плана пользователю необходимо заполнить поля: 
Отчетный период  - поле для указания периода отчета.
Расчет стоимости ТП – поле для указания расчета стоимости ТП.
Шаблон расчетов – поле для выбора справочника из конфигурации финансового плана.
Дата начала действия - поле для указания даты.
На рисунках ниже представлен пример заполнения нового финансового плана. 

![86](/uploads/001/86.png "86")

![87](/uploads/001/87.png "87")

![88](/uploads/001/88.png "88")

![89](/uploads/001/89.png "89")

После создания нового элемента списка, пользователю открывается окно, в котором необходимо выбрать группу тарифов. Система в соответствии с ранее произведенными настройками, произведет расчет с применением базовой ставки по группам услуг.

![90](/uploads/001/90.jpg "90")

После того, как пользователь выберет интересующий его тариф открывается окно, как представлено на рисунке ниже. Основную часть формы занимает табличная форма, колонки которой содержат следующие значения - Тарифы, Наименование показателей, Расчет финансирования, Финансирование по месяцам.  Каждая из данных колонок содержит свои подразделы.

![91](/uploads/001/91.jpg "91")

## 	 Заполнение тарифов.

I. Например, по ВМП планы были созданы в разрезе групп ВМП, соответственно в эти же срезы заполняем тарифы. На рисунке ниже это представлено.

![92](/uploads/001/92.png "92")

![92 1](/uploads/001/92-1.png "92 1")- данная кнопка предназначена для сворачивания окна группы тарифов.

![92 2](/uploads/001/92-2.png "92 2")- данная кнопка разворачивает окно с группами тарифов.
	
	II. Круглосуточный стационар, когда изменяется базовая ставка и управленческий коэффициент. С применением коэффициента регулируется тариф в разрезе КСГ.
	
	![93](/uploads/001/93.png "93")

Конечный расчет объема финансирования в разрезе каждой МО и заполненных срезов.

![94](/uploads/001/94.png "94")

Коэффициенты отображаются в данной части таблицы.

![95](/uploads/001/95.png "95")

Финансирование отображается всего и по месяцам.

![96](/uploads/001/96.png "96")

Так же производится расчет итоговой суммы.

![97](/uploads/001/97.png "97")

![97 1](/uploads/001/97-1.png "97 1") -общая сумма.
![97 2](/uploads/001/97-2.png "97 2") - сумма по факту расчета.
![97 3](/uploads/001/97-3.png "97 3") - оставшаяся сумма.

 ![5 4](/uploads/00003/5-4.jpg "5 4") с помощью данной кнопки все введенные значения сохраняются в БД, рекомендуется периодически сохранять данные. После всех произведенных расчетов необходимо утвердить финансовый план. Для этого необходимо нажать ![97 4](/uploads/001/97-4.png "97 4")
 
# 	Расчет подушевого финансирования.

![100](/uploads/000001/100.png "100")

Для расчета подушевого финансирования на панели структуры программы пользователю необходимо выбрать папку **<Экономика>**. Двойной клик мышью на заголовке раздела, открывает папки. Для работы следует выбрать папку **<Расчет финансового плана>**, двойной клик по ней разворачивает окно, которое показано на рисунке ниже:

![101](/uploads/000001/101.png "101")

Следующим шагом необходимо нажать кнопку ![26 1](/uploads/000001/26-1.jpg "26 1") , в результате чего появится форма с полями для выбора параметров, которые необходимо заполнить, как показано на рисунке ниже. 

![102](/uploads/000001/102.png "102")

**!!! Важно.** В поле  **<Шаблон расчетов>**  следует выбрать конфигурацию финансового плана из расчета за единицу объёма, с добавленными разделами финансового плана по подушевому финансированию.  Далее нажать  **<Ок>**, в результате этого откроется окно следующего вида:

![103](/uploads/000001/103.png "103")

При двойном клике мышью по наименованию группы финансового плана разворачивается окно, как показано на рисунке ниже:

![104](/uploads/000001/104.png "104")

В котором необходимо заполнить все данные и объемы по группам, при заполнении столбцов **<Население>** и **<Объем затраченных средств>**, остальные столбцы **<Объем средств на 1 застр-го>** и **<Коэф. дифференциации>** рассчитываются автоматически. Далее нажать кнопку **<Ок>**, после чего разворачивается окно следующего вида:

![105](/uploads/000001/105.png "105")

Здесь в левой части окна приложения необходимо развернуть таблицу, потянув ее границу кнопкой мыши, для того чтобы ввести все необходимые значения. Если подвести курсор мыши к ячейке, то она становится редактируемой, таким образом можно вводить значения в таблицу, как показано на рисунке ниже:

![106](/uploads/000001/106.png "106")

Также в системе реализован инструмент, позволяющий автоматически заполнять данную табличную форму. 

Численность застрахованного прикрепленного населения по полу, по возрасту и по МО, по данным РЗПН, берется на любую выбранную ТФОМС отчетную дату. Как правило, на последнюю дату ближайшую к событию расчета финансирования по ТПОМС (например, 1 ноября или 1 декабря).

Если подвести курсор на строку наименования МО нажать правую кнопку мыши, в появившемся контекстном меню выбрать **<Открыть карточку МО>**, то разворачивается окно карточки МО.
В верхней части окна приложения есть кнопка **<Редактировать расчёт половозрастных коэф-в диф. подуш. норматива>**, как показано на рисунке ниже:

![107](/uploads/000001/107.png "107")

При нажатии на данную кнопку разворачивается окно для редактирования **<Группы финансового плана>**, аналогично Рис.5. 

Для расчета половозрастных коэффициентов дифференциации подушевого норматива берется любой предыдущий период.  Рекомендовано методикой не менее 6 месяцев, как правило 3 квартала.
Следующим шагом необходимо в верхней части окна табличной формы ввести значение Пнбаз (базовый (средний) подушевой норматив финансирования), как показано на рисунке ниже:

![108](/uploads/000001/108.png "108")

После чего программа  автоматически рассчитает все объемы, при этом изменяется и базовый (средний) поправочный подушевой норматив финансирования (ПНбазПК), а также меняется итоговая сумма в верхнем правом углу окна, как показано на рисунке ниже:

![109](/uploads/000001/109.png "109")

На этом же рисунке видно поле для заполнения поправочного коэффициента, который необходим для приведения в соответствие объема финансирования расчетного с объемом финансирования имеющегося. А также поле для ввода Количества групп.

В таблице расчета финансирования отображены Коэффициенты по федеральной методике, которые в нее подтягиваются автоматически, как это видно на рисунке ниже:

![110](/uploads/000001/110.png "110")

После того как все данные и значения заполнены, программа рассчитывает объемы как годовые, так и по кварталам, распределяя их в соответствии с настройками. Все рассчитанные объемы отображаются в таблице, как показано на рисунке ниже:

![111](/uploads/000001/111.png "111")

# 	Взаимодействие АИС «ОМС Финанс» и АИС «Медэксперт». 

##  Работа с произвольным списком.

Для того чтобы начать работу, пользователю необходимо перейти в АИС «Медэксперт». В главном меню приложения необходимо выбрать пункт **<Сервис>**, одним нажатием левой кнопки мыши открывается список сервисных функций приложения.  Для дальнейшей работы следует выбрать **<Просмотр произвольного списка>**, как показано на рисунке ниже.

![46](/uploads/001/--46.png "46")

## Выбор сущности.

Далее открывается окно <Просмотр произвольного списка>, в котором есть поле для указания сущности.  Каждому разделу в БД должна соответствовать сущность, в которой хранится содержимое раздела  (перечень конкретных значений).  Нужная сущность выбирается из полного списка сущностей системы. Пользователю для работы нужно выбрать сущность «COMPANY_SETTINGS» , наименование этой же сущности на русском языке «Настройки организации». На рисунке ниже представлен пример.

![47](/uploads/001/--47.png "47")

После выбора необходимой сущности открывается форма **<Настройки организации>**. Необходимо развернуть данную вкладку двойным нажатием мыши. 

![48](/uploads/001/--48.png "48")

Выбрав выделенный объект, открывается табличная форма с вкладками, пользователю необходимо выбрать **«Настройки обновления»**. Табличная форма состоит из колонок, которые содержат следующие значения – наименование источника (соответственно - ОМС Финанс ), адрес сервера обновления, код обновления, настройки организации. На рисунке ниже представлено.

![49](/uploads/001/--49.png "49")

## Обновление справочников.

 На главном меню приложения необходимо выбрать пункт **<Сервис>**, одним нажатием левой кнопки мыши открывается список сервисных функций приложения.  Для дальнейшей работы следует выбрать   **<Обновить справочники >**, как показано  на рисунке ниже.

![50](/uploads/001/--50.png "50")

После того, как пользователь произведет все описанные выше действия, откроется окно **<Обновление справочников>**, как показано на рисунке ниже. Для каждого разреза планирования нужен свой справочник.

![51](/uploads/001/--51.png "51")

## Настройка соответствия с ИС Медэксперт.

Данный раздел описывает настройки соответствия с модульным продуктом ИС «Медэксперт». На панели структуры приложения пользователю необходимо выбрать папку **<Справочники>**.  Двойной клик мышью на заголовке раздела, открывает папки, как показано на рисунке. Для работы следует выбрать папку **<Региональные справочники >**, так же **<Планирование>**.

![52](/uploads/001/--52.png "52")

## Расчет факта.

Выберите окно  **<Настройки организации>**,  для расчета факта пользователю следует выбрать вкладку **<Расчет факта>**. После выбора данной вкладки открывается поля с фильтрами. При работе с большими объемами данных возникает необходимость скрыть ненужную в данный момент информацию. Для этого используются фильтры. Фильтр содержит набор условий, которые накладываются на значения атрибутов сущности, представленной в табличной форме. Пользователю необходимо наложить необходимые ему фильтры. На рисунке ниже, приведен пример с наложенными фильтрами.

![53](/uploads/001/--53.png "53")

Следующим действием является – пересчет фактических показателей за период. Для этого пользователю на  главном меню приложения необходимо выбрать пункт **<Действия>**, одним нажатием левой кнопки мыши открывается список сервисных функций приложения.   Для расчета выбрать   **<Пересчитать фактические показатели за год>**.

![54](/uploads/001/--54.png "54")

Аналогично пункту 1 (просмотр произвольного списка), пользователю следует выбрать **<Просмотр произвольного списка>**. Далее выбрать сущность **«SCHET_ITEM_FAKT»** , русское название данной сущности **«Ссылка на факт»**.

![55](/uploads/001/--55.png "55")

Далее открывается окно **<Ссылка на факт>**, которая содержит данные табличной формы с раскрытой панелью фильтра. Пользователю следует наложить фильтр **«Полное сопоставление»**. Данные считаются ошибочными в случае не указания данного фильтра  (полное сопоставление). 

![56](/uploads/001/--56.png "56")


Для выгрузки плановых фактических объемов необходимо загрузить информацию из АИС «МЕД-Эксперт» в АИС «ОМС Финанс». Для этого следует перейти в программу «ОМС Финанс», во вкладке «Сервис» нужно выбрать пункт «Обновить справочники».

![57](/uploads/001/--57.png "57")

Далее в открывшемся окне нужно нажать кнопку «Загрузить». После завершения загрузки данных, следует выбрать «Обновить список».
...
После нажатия кнопки ![73](/uploads/001/--73.png "73")  отображается полный перечень созданных показателей, действующий в указанном периоде времени. 
...
На панели структуры программы пользователю необходимо выбрать папку **<Параметры МО>**. Двойной клик мышью на заголовке раздела, открывает папки. Для работы следует выбрать папку **<Настройка параметров МО>**. 
...
В созданных параметрах необходимо перейти на вкладку «Допустимые разделы планирования», которая показана на рисунке ниже.

![67](/uploads/001/--67.png "67")

С помощью кнопки  ![74](/uploads/001/--74.png "74")   необходимо выбрать  раздел планирования, как показано на рисунке ниже.

![68](/uploads/001/--68.png "68")

После выбора необходимого раздела, появляется окно диалога для подтверждения действия параметров, как представлено на рисунке ниже.

![69](/uploads/001/--69.png "69")

Если необходимо выбрать конкретный подраздел, то пользователю нужно нажать «Нет», после чего появляется окно диалога, как показано на рисунке ниже. 

![70](/uploads/001/--70.png "70")

Если нет необходимости в выборе подраздела, то пользователю нужно нажать «Да». После всех произведенных действий необходимо нажать на клавишу ![5 4](/uploads/00003/5-4.jpg "5 4") , чтобы сохранить информацию.

## Ограничение доступа.

Для того, чтобы ограничить доступ для медицинских организаций, необходимо выбрать вкладку «Доступ» и выбрать действие  «Всем, кроме» или «Только», и нажать кнопку ![74](/uploads/001/--74.png "74") , как показано на рисунке ниже.

![71](/uploads/001/--71.png "71")

После чего открывается справочник медицинских организаций, где пользователю необходимо выбрать медицинские организации, для которых нужно ограничить доступ, как показано на рисунке ниже. 

![72](/uploads/001/--72.png "72")

После всех произведенных действий необходимо нажать на клавишу ![5 4](/uploads/00003/5-4.jpg "5 4") , чтобы сохранить информацию.
...
## 	Доходы
Двойной клик по данной папке разворачивает окно, которое показано на рисунке ниже. Добавление нового элемента списка производится нажатием кнопки  , либо правой кнопки мыши, как показано на рисунке ниже:
...
Чтобы внести изменения на вкладке, необходимо воспользоваться кнопками: ![82](/uploads/001/--82.png "82") .

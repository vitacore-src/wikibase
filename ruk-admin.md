...
Во вкладке «Счета-реестры» Области отображения данных следует нажать на наименование любого атрибута, расположенного внизу этой вкладки (порядок открытия вкладки «Счета-реестры» описан в разделе 4.9.2). В приведенном примере выбрано название атрибута «Номер счета», выделено красным цветом (Рис. 33). В открывшемся контекстном меню необходимо нажать на пункт «Выбрать атрибуты…», который выделен красным цветом (Рис. 33). При этом откроется форма настройки атрибутов - дополнительное окно «Счет-реестр|operator» (Рис. 35). Наименование сущности «Счет-реестр» и контекста «operator» будет служить ключами при указании переноса необходимой формы по локальным клиентам системы.
